...
(define (get-pref-list pref person)
   (cdr (car (filter (lambda (l) (equal? (car l) person)) pref) )))
...
(define (get-partner engagements person)
  (define  rezultat
    (find-first (lambda(couple) (equal? (car couple) person)) engagements))
  (if rezultat
      (cdr rezultat)
      #f)
  )
...
(define (update-engagements engagements p1 p2)
  (change-first (lambda (couple) (equal? (car couple) p1)) engagements (cons p1 p2) ))
...
(define (reverse-eng engagements)
  (foldr (lambda(couple acc)
           (cons (cons (cdr couple) (car couple)) acc))
         '()
         engagements))

(define (stable-match? engagements mpref wpref)
 (not (ormap (lambda(couple)
               (better-match-exists?  (car couple) (cdr couple) (get-pref-list wpref (car couple)) mpref (reverse-eng engagements)) )
             engagements)))
