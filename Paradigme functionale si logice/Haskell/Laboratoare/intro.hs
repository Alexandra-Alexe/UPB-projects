...
reverseList3 l = case null l of
    True -> l
    False -> (reverseList (tail l)) ++ [(head l)]
...
-- foldl
reverseList5 :: [a] -> [a]
reverseList5 l = (++) [] (reverseList l)

-- foldr
reverseList6 :: [a] -> [a]
reverseList6 l = (++) [] (reverseList l)
...
numToBase4 n b = case n == 0 of
    True -> []
    False -> (numToBase (quot n b) b) ++ [(mod n b)]
...
removeDuplicatesLeft l = (reverse 
    (foldl (\acc x -> if elem x acc then acc else x : acc  ) 
        [] l))
...
removeDuplicatesRight l = (foldr (\x acc -> if elem x acc then acc else x : acc) 
        [] l)
