...
instance Eq a => Eq (BST a) where
    (==) (BSTNod elem1 lt1 rt1) (BSTNod elem2 lt2 rt2) = elem1 == elem2 && lt1 == lt2 && rt1 == rt2
    (==) BSTNil BSTNil = True
    (==) _ _ = False
...
printLevel :: Show a => Int -> Char -> BST a -> [Char]
printLevel _ _ BSTNil = ""
printLevel level tab (BSTNod r lf rg) = replicate level tab ++ show root ++ "\n" 
    ++ printLevel (level + 1) tab lf
    ++ printLevel (level + 1) tab rg

instance Show a => Show (BST a) where   
    show BSTNil = ""
    show node@(BSTNod root left right) = printLevel 0 '\t' node
...
instance Invertible (BST a) where
    invert (BSTNod root left right) = BSTNod root (invert left) (invert right)
...
instance Functor BST where
    fmap f (BSTNod root left right) = BSTNod (f root) (fmap f left) (fmap f right) 
