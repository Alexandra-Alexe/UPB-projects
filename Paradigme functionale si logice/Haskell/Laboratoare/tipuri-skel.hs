...
insertElem root@(BSTNod valoare stg dr) elem
  | elem == valoare = root
  | elem > valoare = BSTNod valoare stg (insertElem dr elem)
  | elem < valoare = BSTNod valoare (insertElem stg elem) dr

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem (BSTNod valoare stg dr) elem 
  | elem == valoare = Just valoare
  | elem > valoare = findElem dr elem
  | elem < valoare = findElem stg elem

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNod elem stg dr) = inorder stg ++ [elem] ++ inorder dr

size :: BST a -> Int
size BSTNil = 0
size (BSTNod _ stg dr) = size stg + 1 + size dr

height :: BST a -> Int
height BSTNil = 0
height (BSTNod _ stg dr) = 1 + max (height stg) (height dr)

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil = False
isBalanced (BSTNod elem stg dr)
  | not (isBalanced stg) = False
  | not (isBalanced dr) = False
  | otherwise  = True
