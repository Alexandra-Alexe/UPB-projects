...
attach tree1 tree2
    | isEmptyTree tree1 = tree2
    | isEmptyTree tree2 = tree1
...
insert prio key (BinomialHeap size trees) = 
    BinomialHeap (size + 1) (insertTree (Node prio key []) trees)
...
isolate _ [] = []
isolate placeHolder (x:xs) = reverse $ helper placeHolder (x:xs) []
  where
    helper :: a -> [a] -> [a] -> [(a, [a])]
    helper _ [] _ = []
    helper placeHolder (y:ys) acc = (y, reverse (ys' ++ acc)) : helper placeHolder ys (y:acc)
      where
        ys' = if compareWithPlaceholder placeHolder y then ys else placeHolder : ys

    compareWithPlaceholder :: a -> a -> Bool
    compareWithPlaceholder x y = compareWithPlaceholder' y
      where
        compareWithPlaceholder' z = z == x
